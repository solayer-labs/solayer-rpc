...
impl ThreadAwareAccountLocks {
    /// Creates a new `ThreadAwareAccountLocks` with the given number of
    /// threads.
    pub(crate) fn new(num_threads: usize) -> Self {
        assert!(num_threads > 0, "num threads must be > 0");
        assert!(num_threads <= MAX_THREADS, "num threads must be <= {MAX_THREADS}");
    ...
    /// `allowed_threads` is a set of threads that the caller restricts locking
    /// to. If accounts are schedulable, then they are locked for the thread
    ...
    /// Returns `ThreadSet` of schedulable threads for the given readable
    /// account.
    fn read_schedulable_threads(&self, account: &Pubkey) -> ThreadSet {
        self.schedulable_threads::<false>(account)
    }

    /// Returns `ThreadSet` of schedulable threads for the given writable
    /// account.
    ...
    /// If only write-locked, then only the thread holding the write lock is
    /// schedulable. If a mix of locks, then only the write thread is
    /// schedulable. If only read-locked, the only write-schedulable thread
    /// is if a single thread   holds all read locks. Otherwise, no threads
    /// are write-schedulable. If only read-locked, all threads are
    /// read-schedulable.
    fn schedulable_threads<const WRITE: bool>(&self, account: &Pubkey) -> ThreadSet {
        match self.locks.get(account) {
            ...
            Some(AccountLocks {
                ...
                assert_eq!(read_locks.thread_set.only_one_contained(), Some(write_locks.thread_id));
    ...
    fn lock_accounts<'a>(
        ...
        assert!(thread_id < self.num_threads, "thread_id must be < num_threads");
        for account in write_account_locks {
    ...
    fn write_lock_account(&mut self, account: &Pubkey, thread_id: ThreadId) {
        ...
        if let Some(write_locks) = write_locks {
            assert_eq!(
                ...
                "outstanding write lock must be on same thread {account}"
            );
