...
use infinisvm_types::transaction_batch_id::TransactionBatchId;
use infinisvm_types::transaction_id::TransactionId;
...
pub(crate) type ScheduledJob = (Vec<SanitizedTransaction>, Vec<TransactionId>, TransactionBatchId);
...
impl<'a> Worker<'a> {
    ...
    pub(crate) fn run_loop(
        ...
        loop {
            let (sanitized_txs, transaction_ids, batch_id) = match self.job_receiver.recv() {
                Ok(payload) => payload,
                Err(err) => {
                    error!(
                        worker_id = self.id,
                        error = ?err,
                        "job receiver disconnected; worker loop exiting"
                    );
                    break;
                }
            };
            ...
            let (slot, blockhash, timestamp, parent_blockhash) = bank_read.get_latest_slot_hash_timestamp_parent();
            ...
            // Send results through channel; exit cleanly if the scheduler side shuts down.
            if let Err(err) = callback_sender.send((commit_batch, batch_id)) {
                error!(
                    worker_id = self.id,
                    error = ?err,
                    "callback channel disconnected (len={job_len}); worker loop exiting"
                );
                break;
            }
