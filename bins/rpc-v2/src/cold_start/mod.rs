...
pub async fn cold_start(
    ...
    slot_receivers: Vec<mpsc::Receiver<RawSlot>>,
    ...
    let tx_handles = tx::spawn_tx_processors(tx::TxProcessorConfig {
        ...
        pending_signature_count: pending_signature_count.clone(),
    });
    handles.extend(tx_handles);

    let slot_handles = slot::spawn_slot_processors(slot::SlotProcessorConfig {
        receivers: slot_receivers,
        db_chain: db_chain_ref.clone(),
        bank: bank.clone(),
        indexer: indexer.clone(),
        num_slots: num_slots.clone(),
        current_slot: current_slot.clone(),
        finalized_job_ids: finalized_job_ids.clone(),
        blockhash_to_signatures: blockhash_to_signatures.clone(),
        pending_signature_count: pending_signature_count.clone(),
    });
    handles.extend(slot_handles);
    ...
    let sampler_handle = sampler::spawn_sampler(
        samples.clone(),
        total_transaction_count.clone(),
        num_transactions.clone(),
        num_slots.clone(),
        current_slot.clone(),
    );
    handles.push(tokio::spawn(async move {
        sampler_handle.await.expect("Sampler task exited with error")
    }));
